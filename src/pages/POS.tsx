    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="flex bg-stone-200/50 p-1.5 rounded-2xl w-fit self-center sm:self-start">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold transition-all ${activeTab === 'rental' ? 'bg-emerald-600 text-white shadow-lg' : 'text-stone-500 hover:text-stone-700'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold transition-all ${activeTab === 'return' ? 'bg-orange-600 text-white shadow-lg' : 'text-stone-500 hover:text-stone-700'}`}
        >
          <History size={18} />
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
                {/* Categories */}
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${selectedCategory === 'all' ? 'bg-stone-900 text-white' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'}`}
                  >
                    Semua Item
                  </button>
                  {categories.map(cat => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${selectedCategory === cat.id.toString() ? 'bg-stone-900 text-white' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'}`}
                    >
                      {cat.name}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                  {filteredItems.map(item => (
                    ...
                    >
                      ...
                      <h3 className="font-bold text-stone-900 text-sm line-clamp-2 min-h-[40px] uppercase leading-tight">{item.name}</h3>
                      <div className="mt-auto pt-3 flex items-end justify-between border-t border-stone-50">
                        <div>
                          <p className="text-[10px] text-stone-400 font-bold uppercase tracking-widest">{item.categoryName}</p>
                          <p className="font-black text-emerald-600 text-sm mt-0.5">{formatCurrency(item.dailyPrice)}</p>
                        </div>
                        ...
                        {isPaymentDropdownOpen && (
                          <div className="absolute bottom-full mb-1 left-0 w-full bg-white shadow-xl rounded-xl border border-stone-200 z-50 overflow-hidden py-1">
                            {paymentMethods.map(p => (
                              <button key={p.id} onClick={() => { setPaymentMethod(p.name); setIsPaymentDropdownOpen(false); }} className="w-full px-3 py-2 text-left text-xs font-bold hover:bg-emerald-50 text-stone-700">{p.name}</button>
                            ))}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="relative">
                    ...
                    {isDiscountDropdownOpen && (
                      <div className="absolute bottom-full mb-1 left-0 w-full bg-white shadow-xl rounded-xl border border-stone-200 z-50 overflow-hidden py-1 max-h-48 overflow-y-auto">
                        <button onClick={() => { setDiscount(0); setIsDiscountDropdownOpen(false); }} className="w-full px-3 py-2 text-left text-xs font-bold hover:bg-stone-50">Tanpa Diskon</button>
                        {discounts.map(d => (
                          <button key={d.id} onClick={() => { setDiscount(d.percentage); setIsDiscountDropdownOpen(false); }} className="w-full px-3 py-2 text-left text-xs font-bold hover:bg-emerald-50 text-stone-700 flex justify-between">
      ...
      <AnimatePresence>
        ...
        {showConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden">
              <div className="p-8 pb-4">
                <h2 className="text-2xl font-black text-stone-900 mb-6 flex items-center gap-3 tracking-tighter"><AlertCircle className="text-amber-500" /> KONFIRMASI RENTAL</h2>
                <div className="bg-stone-50 p-6 rounded-3xl space-y-3 mb-6 border border-stone-200 shadow-inner">
                  <div className="flex justify-between text-xs uppercase font-black"><span className="text-stone-400">Penyewa</span> <span className="text-stone-900">{customerName}</span></div>
                  <div className="flex justify-between text-xs uppercase font-black"><span className="text-stone-400">No. HP</span> <span className="text-stone-900">{customerPhone}</span></div>
                  <div className="flex justify-between text-xs uppercase font-black"><span className="text-stone-400">Jaminan</span> <span className="text-stone-900">{guarantee}</span></div>
                  <div className="flex justify-between text-xs uppercase font-black"><span className="text-stone-400">Durasi</span> <span className="text-stone-900">{durationDays} Hari</span></div>
                  <div className="flex justify-between text-xs uppercase font-black text-emerald-600 border-t border-stone-200 pt-3"><span className="text-stone-400">Total Bayar</span> <span className="text-lg font-black">{formatCurrency(totalAmount)}</span></div>
                </div>
                <p className="text-[10px] text-stone-400 text-center font-black uppercase tracking-widest px-4">Setelah dikonfirmasi, stok barang akan otomatis berkurang.</p>
              </div>
              <div className="p-6 bg-stone-50 flex gap-4 border-t border-stone-100">
                <button onClick={() => setShowConfirmModal(false)} className="flex-1 py-4 font-black text-stone-400 tracking-widest hover:text-stone-600 transition-colors">BATAL</button>
                <button onClick={executeSubmit} disabled={submitting} className="flex-[2] py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-lg shadow-emerald-500/20 active:scale-95 transition-all uppercase tracking-widest">{submitting ? 'MEMPROSES...' : 'KONFIRMASI SEKARANG'}</button>
              </div>
            </motion.div>
          </div>
        )}
        ...
        {showSuccessModal && lastTransactionData && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/90 backdrop-blur-xl">
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white rounded-[40px] w-full max-w-lg p-0 text-left shadow-2xl overflow-hidden">
              <div className="bg-emerald-600 p-8 text-center text-white relative">
                <div className="w-16 h-16 bg-white/20 text-white rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-white/50"><CheckCircle2 size={32} /></div>
                <h2 className="text-2xl font-black tracking-tighter">SEWA TERDAFTAR!</h2>
                <p className="text-emerald-100 text-xs font-bold uppercase tracking-widest mt-1">Transaksi Berhasil Disimpan</p>
                <button onClick={() => setShowSuccessModal(false)} className="absolute top-4 right-4 p-2 bg-black/10 hover:bg-black/20 rounded-full text-white transition-colors"><X size={20} /></button>
              </div>

              <div className="p-6 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-stone-200">
                {/* Customer Details */}
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Penyewa</p>
                    <p className="font-black text-stone-900 text-base">{lastTransactionData.customerName}</p>
                    <p className="text-xs text-stone-500 font-bold">{lastTransactionData.customerPhone}</p>
                  </div>
                  <div className="space-y-1 text-right">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Jaminan</p>
                    <p className="font-black text-stone-900 text-base">{lastTransactionData.guarantee || '-'}</p>
                  </div>
                </div>

                {/* Rental Details */}
                <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100 mb-6 flex justify-between items-center">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Periode Sewa</p>
                    <p className="text-xs font-black text-stone-800 uppercase">{format(new Date(lastTransactionData.startDate), 'dd MMM')} - {format(new Date(lastTransactionData.endDate), 'dd MMM yyyy')}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Durasi</p>
                    <p className="text-lg font-black text-emerald-600 leading-none">{lastTransactionData.durationDays} Hari</p>
                  </div>
                </div>

                {/* Item List */}
                <div className="mb-6">
                  <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-3">Detail Barang</p>
                  <div className="space-y-2">
                    {lastTransactionData.items.map((item: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center text-xs font-bold p-2 bg-stone-50 rounded-xl border border-stone-100">
                ...
                <div className="border-t border-dashed border-stone-200 pt-4 space-y-2">
                  <div className="flex justify-between text-xs font-bold text-stone-500">
                    <span>Subtotal</span>
                    <span>{formatCurrency(lastTransactionData.subtotal)}</span>
                  </div>
                  {lastTransactionData.discount > 0 && (
                    <div className="flex justify-between text-xs font-bold text-red-500">
                      <span>Potongan ({lastTransactionData.discount}%)</span>
                      <span>-{formatCurrency(lastTransactionData.discountAmount)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-sm font-black text-stone-900 uppercase tracking-tighter">Total Akhir ({lastTransactionData.paymentMethod})</span>
                    <span className="text-2xl font-black text-emerald-600 tracking-tight">{formatCurrency(lastTransactionAmount)}</span>
                  </div>
                </div>
              </div>

              <div className="p-6 bg-stone-50 flex flex-col md:flex-row gap-3 border-t border-stone-100">
                <button onClick={handleSendInvoiceWA} className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"><MessageCircle size={18} /> WA KIRIM</button>
                <button onClick={handlePrint} className="flex-1 py-4 bg-stone-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-all"><Printer size={18} /> CETAK STRUK</button>
                <button onClick={() => setShowSuccessModal(false)} className="md:w-auto py-4 px-6 bg-white border border-stone-200 text-stone-400 rounded-2xl font-black text-xs uppercase tracking-widest transition-all">TUTUP</button>
              </div>
            </motion.div>
          </div>
        )}
